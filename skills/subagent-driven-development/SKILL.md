...
If the user says "change A to B" during execution, do not just patch the current task. Propagate the change to all artifacts:
...
The user's request steers the entire process, not just the current task. A mid-execution change from the user has the same authority as an initial requirement stated during brainstorming. Subsequent subagents must receive the updated context.
