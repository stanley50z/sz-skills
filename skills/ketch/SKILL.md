---
name: ketch
description: "Research skill for ketch — a stateless CLI for web search, OSS code search, library docs, page scraping, and site crawling. In the Pi coding agent, use ketch as the default for live-source research. In Codex, Claude Code, OpenCode, and other harnesses with native web search or browsing tools, prefer those native tools for ordinary web research; use ketch only when the user explicitly requests it or its code/docs/scrape/crawl surfaces are specifically useful. Also use when installing or configuring ketch. Not for local codebase search, private repos, or pages behind auth."
---

# Ketch

When harness routing selects ketch, route the live-source question to one of its five research surfaces — search, code, docs, scrape, crawl — over the transport the operator gave you, with a token budget on every fetch and a source URL on every claim. ketch is one stateless binary — call, result, exit — with web search, OSS code grep, curated library docs, and page/site extraction together.

## Harness routing: decide before transport

1. **Pi coding agent:** default to ketch for live web research, OSS code search, library docs, scraping, and crawling.
2. **Codex, Claude Code, OpenCode, or another harness with native web search/browsing:** prefer the harness's native tools for ordinary web research. Use ketch when the user explicitly asks for it or when its `code`, `docs`, `scrape`, or `crawl` surface is specifically better suited to the request. Do not replace native web search merely because ketch is installed.
3. **Unknown harness:** if native live-source tools are available, follow rule 2; otherwise follow the Pi default and use ketch.

After this choice selects ketch, use the transport rules below.
...
- Default after ketch is selected: answer one question with one or two routed calls. Use the surface routing table, token budgets, and error control flow below.
...
1. **Respect the harness boundary.** Pi defaults to ketch. Harnesses with native web tools keep those tools as their ordinary web-research default unless the user explicitly chooses ketch or a ketch-specific surface is a better fit.
...
3. **Bound every fetch.** `max_chars` 4000–8000 plus `trim` on any scrape of a page you have not seen — an unguarded page can cost ~25k tokens. Skipping the cap requires a stated one-line reason ("known ~200-word page").
4. **Cite every claim.** A research synthesis without source URLs is not a deliverable.
5. **Error prefixes are control flow.** Classify before reacting. Never retry `[validation]` or `[not_found]` unchanged.
6. **Propose, then mutate.** `config set`, `browser install`, docker runs, installs — only after the operator confirms the exact command. Never touch a value that is already configured and working.
7. **The binary outranks this file.** `ketch config` and `--help` are ground truth; where they disagree with a table here, trust the binary and flag the skill as drifted.
...
**BAD:** In Codex, Claude Code, or OpenCode, route a routine web query through ketch solely because the skill is installed.
**GOOD:** Pi defaults to ketch; harnesses with native web tools keep their native search default and select ketch only explicitly or for a better-suited ketch surface.
