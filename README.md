...
| [to-tickets](skills/to-tickets/) | Break a spec into tracer-bullet tickets with blocking edges | ↳ `skills/engineering/to-tickets` | `Requirement:` spec-trace field, visual-only UI acceptance criteria, cross-phase change propagation, HTML plan companion |
| [implement](skills/implement/) | Implement one ticket at a time with TDD, code review, and commit | ↳ `skills/engineering/implement` | No-fallback hard gate, suggest-don't-auto-apply, version-upgrade test cleanup, visual-only UI checks, cross-phase change propagation, local skill names |
